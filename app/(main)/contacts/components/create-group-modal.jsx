...
export function CreateGroupModal({ isOpen, onClose, onSuccess }) {
  ...
  const removeMember = (identifier) => {
    setSelectedMembers(
      selectedMembers.filter(
        (m) => m.email !== identifier && m.id !== identifier
      )
    );
  };

  // -------------------------------------------------------------
  // SUBMIT — BUILD EXACT ARRAY THAT BACKEND EXPECTS
  // -------------------------------------------------------------
  const onSubmit = async (data) => {
    try {
      const members = selectedMembers.map((member) => ({
        email: member.email,
        role: "member",
        userId: member.id || null, // real user OR email-only user
      }));
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              {selectedMembers.map((member) => (
                <Badge key={member.email} variant="secondary" className="px-3 py-1">
                  <Avatar className="h-5 w-5 mr-2">
            ...
            {selectedMembers.length === 0 && (
              <p className="text-sm text-amber-600">Add at least one member</p>
            )}
          </div>
